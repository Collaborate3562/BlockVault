...
export default function Vesting() {
  ...
  async function fetchAccountData() {
    ...
    tokenInstance_temp.options.address = "0x2Eac5f7a6F5f01B9d32b4c10DEA747Ba98049D71";
  ...
  const Cliam_90 = async => {
    if(tokenInstance && accounts.length > 0){
      if(isVested_90){
        await tokenInstance.methods.cliam_vest().send({from:accounts[0]});
      } else {
          store.addNotification({
            title: "Error",
            message: "You can't cliam",
              ...
              <div className="col-lg-4 offset-lg-1 col-md-6 wow fadeInUp mt-5" data-wow-duration="1.5s">
                <div className="xs-count-down">
                  <div className="btn-wrapper text-center">
                    ...
                    {isVested_90 && (<button className="btn btn-primary" onClick={() => Cliam_90()}>Cliam BV</button>)}
                  </div>
                </div>
              </div>
